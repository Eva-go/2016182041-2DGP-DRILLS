...
def draw_curve_4_points(p1, p2, p3, p4):
    ...
    for i in range(0, 100, 2):
        t = i / 100
        x = ((-t**3 + 2*t**2 - t)*p3[0] + (3*t**3 - 5*t**2 + 2)*p4[0] + (-3*t**3 + 4*t**2 + t)*p1[0] + (t**3 - t**2)*p1[0])/2
        y = ((-t**3 + 2*t**2 - t)*p3[1] + (3*t**3 - 5*t**2 + 2)*p4[1] + (-3*t**3 + 4*t**2 + t)*p1[1] + (t**3 - t**2)*p1[1])/2
        draw_point((x, y))
    draw_point(p4)
...
while True:
    n = 0
    draw_curve_4_points (points[n],points[n-3],points[n-2],points[n-1])
    n=(n+1)%size
