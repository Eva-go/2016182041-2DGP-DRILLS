...
BIRD_SPEE_KMPH = 20.0
BIRD_SPEED_MPM = (BIRD_SPEE_KMPH * 1000.0 / 60.0)
...
class Bird:
    def __init__(self):
        self.x, self.y = 918, 200
        self.image = load_image('bird_animation.png')
        self.frame = 0
        self.velocity = 0

    def draw(self):
        self.image.clip_draw(int(self.frame) * 183, 0, 183, 168, self.x, self.y)

    def update(self):

        self.x += self.velocity * game_framework.frame_time
        self.velocity + BIRD_SPEED_PPS
        self.frame = (self.frame + BIRD_FRAMES_PER_ACTION * BIRD_ACTION_PER_TIME * game_framework.frame_time) % 4
        #self.velocity -= BIRD_SPEED_PPS
        #   self.velocity-=BIRD_SPEED_PPS
